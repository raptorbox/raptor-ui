...
var colors = ['rgb(65,184,131)', 'rgb(228,102,81)', 'rgb(0,216,255)', 'rgb(221,27,22)', 'rgb(225,225,0)']

export default {
  methods: {
    load (chartsData) {
      this.$raptor.Inventory().read(chartsData.device).then((device) => {
        // console.log(device)
    ...
    subscribeStream (chartsData, stream) {
      let context = this
      this.$raptor.Stream().list(stream, 0, 100, 'timestamp,desc').then((streams) => {
        // console.log(streams)
    ...
    loadDatasets (chartsData) {
      ...
      for (var i = 0; i < chartsData.chartData.length; i++) {
        this.$raptor.Inventory().read(chartsData.chartData[i].device).then((device) => {
          // console.log(device)
    ...
    getDataForStream (chartsData, stream) {
      ...
      this.$raptor.Stream().search(stream, query).then((streams) => {
        // console.log(streams)
        if (streams.length > 0) {
    ...
    searchDataApi (chartsData, stream, query, callback) {
      ...
      // console.log(stream)
      let context = this
      this.$raptor.Stream().search(stream, query).then((stream) => {
        chartsData.apiCalls--
      ...
      this.searchDataApi(chartsData, stream, query, function (streams) {
        ...
        if (streams && (streams[total - 1].timestamp * 1000) < endDate && total === 500) {
          ...
          if (chartsData.datasets.length > 0) {
            for (var j = 0; j < chartsData.datasets.length; j++) {
              if (chartsData.datasets[j].device.id === streams[0].json.deviceId) {
                chartsData.datasets[j].selectedStreamData = chartsData.datasets[j].selectedStreamData.concat(streams)
              }
            }
          } else {
            chartsData.selectedStreamData = chartsData.selectedStreamData.concat(streams)
